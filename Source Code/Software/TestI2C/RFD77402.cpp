...
bool RFD77402::begin(I2C &i2cPort)
{
  ...
  if (AgoToOnMode() == false) return (false); //MCPU never turned on
  ...
  if (AgoToOnMode() == false) return (false); //Error - MCPU never turned on
...
uint16_t RFD77402::AgetMailbox(void)
{
...
{
  if (AgoToOnMode() == false) return (false); //Error - sensor timed out before getting to On Mode
  ...
  while (1)
  {
    ...
    AgetMailbox(); //Throw it out. Just read to clear the register.
  ...
  for (uint8_t message = 0 ; message < 27 ; message++)
  {
    ...
    uint16_t incoming = AgetMailbox(); //Get 16-bit message
